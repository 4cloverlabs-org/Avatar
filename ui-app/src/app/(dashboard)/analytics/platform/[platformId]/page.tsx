...
export default function PlatformDeepDivePage() {
  ...
  const platformConfig: Record<string, { name: string, color: string, bg: string }> = {
    ...
    tiktok: { name: 'TikTok', color: '#000000', bg: 'var(--muted-bg)' },
    twitter: { name: 'Twitter/X', color: '#1da1f2', bg: '#f0f9ff' },
    facebook: { name: 'Facebook', color: '#1877f2', bg: 'var(--muted-bg)' }
  };
  ...
  if (loading) {
    return (
      <div style={{ padding: '40px', display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}>
        <div style={{ color: 'var(--text-muted)', fontSize: 16, fontWeight: 500 }}>Loading {config.name} insights...</div>
      </div>
    );
  }
  ...
  return (
    <div className="custom-scrollbar" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
        <div style={{ width: 48, height: 48, borderRadius: '12px', background: config.color, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--panel-bg)', fontSize: 24, fontWeight: 800 }}>
          {config.name.charAt(0)}
        </div>
        <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--foreground)', letterSpacing: '-0.5px' }}>
      ...
      <div className="grid-row-1">
        <div className="summary-card">
          ...
          <div>
            <div style={{ color: 'var(--text-muted)', fontSize: 13, fontWeight: 600, marginBottom: 4 }}>Total Followers</div>
        ...
        <div className="summary-card">
          ...
          <div>
            <div style={{ color: 'var(--text-muted)', fontSize: 13, fontWeight: 600, marginBottom: 4 }}>Avg Engagement Rate</div>
            <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--foreground)' }}>
              8.4%
            </div>
          </div>
        ...
        <div className="summary-card">
          ...
          <div>
            <div style={{ color: 'var(--text-muted)', fontSize: 13, fontWeight: 600, marginBottom: 4 }}>Total Posts Published</div>
            <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--foreground)' }}>
              142
            </div>
          </div>
      ...
      <div className="glass-card" style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--foreground)', marginBottom: 24 }}>Audience Growth (Last 30 Days)</div>
        <div style={{ height: 300, width: '100%' }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={followerData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke='var(--muted-bg)' />
              <XAxis dataKey="date" axisLine={{ stroke: 'var(--panel-border)' }} tickLine={false} tick={{ fill: 'var(--text-muted)', fontSize: 12 }} dy={10} />
              <YAxis axisLine={false} tickLine={false} tick={{ fill: 'var(--text-muted)', fontSize: 12 }} dx={-10} tickFormatter={(v) => `${(v/1000).toFixed(1)}k`} />
              <Tooltip 
      ...
      <div className="grid-row-2">
        <div className="glass-card">
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--foreground)' }}>Demographics (Age)</div>
          </div>
              ...
              {demoData.map((d, i) => (
                <div key={d.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 13, color: 'var(--text-muted)', fontWeight: 500 }}>{d.name}</span>
                  </div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--foreground)' }}>{d.value}%</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="glass-card">
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--foreground)' }}>Top Regions</div>
          </div>
          <div style={{ height: 220, width: '100%' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={geoData} layout="vertical" margin={{ top: 0, right: 30, left: 30, bottom: 0 }}>
                ...
                <YAxis dataKey="country" type="category" axisLine={false} tickLine={false} tick={{ fill: 'var(--text-muted)', fontSize: 12, fontWeight: 500 }} />
                <Tooltip cursor={{fill: 'var(--muted-bg)'}} contentStyle={{ borderRadius: 8, border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} formatter={(v: any) => [`${v}%`, 'Audience']} />
      ...
      <div className="grid-row-2">
        <div className="glass-card">
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--foreground)' }}>Best Time to Post</div>
          </div>
          <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 24 }}>Average engagement mapped across times of day.</div>
          
          <div style={{ height: 250, width: '100%' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={timeToPostData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke='var(--muted-bg)' />
                <XAxis dataKey="hour" axisLine={false} tickLine={false} tick={{ fill: 'var(--text-muted)', fontSize: 12 }} dy={10} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: 'var(--text-muted)', fontSize: 12 }} dx={-10} />
                <Tooltip cursor={{fill: 'var(--muted-bg)'}} contentStyle={{ borderRadius: 8, border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} formatter={(v: any) => [`${v}%`, 'Engagement Potential']} />
                <Bar dataKey="engagement" fill="#3b82f6" radius={[4, 4, 0, 0]} barSize={32}>
                  {timeToPostData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={entry.engagement > 70 ? config.color : 'var(--panel-border)'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="glass-card">
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--foreground)' }}>Top Performing Content</div>
          </div>
          <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 24 }}>Highest engaged videos on {config.name}.</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {topVideos.map((vid, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px', background: 'var(--muted-bg)', borderRadius: '12px', border: '1px solid var(--panel-border)' }}>
                <div style={{ width: 40, height: 40, borderRadius: '8px', background: config.color, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--panel-bg)', opacity: 0.9 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>{vid.date}</div>
                </div>
